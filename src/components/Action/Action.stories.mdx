...
<Preview>
  <Story name='States'>
    {() => {
      ...
      const loadingStates = {
        Idle: { icon: 'Plus', text: 'Do something' },
        Loading: { icon: 'Loading', text: 'Loading...' },
        Done: { icon: 'Tick', text: 'Done' },
        Retry: { icon: 'Cross', text: 'Retry' },
      }
      const handleClick = () => {
        dispatchEvent('LOAD')
        setTimeout(() => {
      ...
      return (
        <Action icon={{ name: loadingStates[loadingState].icon, active: true }} onClick={handleClick}>{loadingStates[loadingState].text}</Action>
      )
    }}
  </Story>
</Preview>
