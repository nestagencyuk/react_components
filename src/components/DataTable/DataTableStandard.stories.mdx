...
import { testSKUS, testDescriptions, testUOMs, testQuantities, testBatch, configStandard, configNoControls } from './DataTableConfig.ts'
...
| `'SORT'` | Fired when the sort button is clicked |
| `'PREV_PAGE'` | Fired when the "prev page" button is clicked |
| `'NEXT_PAGE'` | Fired when the "next page" button is clicked |
...
<Preview>
  <Story name="Without Controls">
    {() => {
      const [data, setData] = useState(configNoControls.data)
      const [loadingState, dispatchEvent] = useStateMachine({ machine: { ...loading, initial: 'Loading' } })
      const chance = new (require('chance'))()
      useEffect(() => {
        const fakeData = Array.from(Array(1000).keys()).map(x => ({
      ...
      return (
        <Fragment>
          <DataTable
            loadingState={loadingState}
            controls={configNoControls.controls}
            header={configNoControls.header} 
            rows={configNoControls.rows}
            data={data}
          />
          <div style={{ marginBottom: '50px' }} />
        </Fragment>
