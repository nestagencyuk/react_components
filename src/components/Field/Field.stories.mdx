...
## Required
By default, a field is optional, however a `required` prop can be passed in.
...
<Preview>
  <Story name="Required">
    {() => {
      ...
      return (
        <Fragment>
          <Field id="field-base" label="Name" value={value} required onChange={setValue} />
          <Button onClick={() => alert(`Form data: ${value}`)}>Submit</Button>
        </Fragment>
      )
    }}
...
## Number
Pass `Number` to the `type` prop to render a Number input. 

It also accepts numbers passed to the optional `minValue` and `maxValue` props.
...
<Preview>
  <Story name="Number">
    {() => {
      ...
      return (
        <Fragment>
          <Field id="field-number" label="Message" type="Number" value={value} onChange={setValue} minValue={5} maxValue={10} />
          <Button onClick={() => alert(`Form data: ${value}`)}>Submit</Button>
        </Fragment>
      )
    }}
...
## Switch
Pass `Switch` to the `type` prop to render a Radio component
...
<Preview>
  <Story name="Switch">
    {() => {
      const [value, setValue] = useState(false)
      return (
        <Field id="field-switch" label="Switch" type="Switch" value={value} onChange={setValue} />
      )
    }}
  </Story>
</Preview>
...
<Preview>
  <Story name="Upload">
    {() => {
      ...
      return (
        <Fragment>
          <Field id="file-upload" type="Upload" label="Upload" loadingState={loadingState} onChange={mockUpload} />
        </Fragment>
      )
    }}
  </Story>
