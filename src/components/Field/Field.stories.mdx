...
## DatePicker
Pass `datePicker` to the `type` prop to render a Date Picker.
...
<Preview>
  <Story name="DatePicker">
    {() => {
      ...
      return (
        <Fragment>
          <Field type="DatePicker" label={`${value}`} selected={value} placeholderText="Select a date or type" onChange={setValue} />
        </Fragment>
      )
    }}
  </Story>
