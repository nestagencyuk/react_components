    ...
    {() => {
      ...
      return (
        <div>
          ...
          <Checkbox id='checkbox' value={values.includes('dog')} onChange={(value) => handleChange('dog', value)} />
          <Checkbox id='checkbox' value={values.includes('horse')} onChange={(value) => handleChange('horse', value)} />
        </div>
      )
    }}
  </Story>
